use bdEscola
	...
	--4--
	select count(tbAluno.codAluno) as 'Matrículas realizadas em maio' from tbAluno
		inner join tbMatricula on tbMatricula.codAluno = tbAluno.codAluno
	...
	--8--
	select nomeAluno as 'Aluno', FORMAT(dataMatricula, 'dd/MM/yyyy') as 'Data de matrícula' from tbAluno
		inner join tbMatricula on tbMatricula.codAluno = tbAluno.codAluno
			order by dataMatricula

	--9-- - Alessandro não ta nem matriculado -
	select nomeAluno as 'Aluno', nomeCurso as 'Curso' from tbAluno
		inner join tbMatricula on tbMatricula.codAluno = tbAluno.codAluno
		inner join tbTurma on tbTurma.codTurma = tbMatricula.codTurma
		inner join tbCurso on tbCurso.codCurso = tbTurma.codCurso
	...
	--10--
	select count(codMatricula) as 'Matrículas realizadas em 2016' from tbMatricula
		where datepart(year, dataMatricula) = 2016

	--11--
	select count(codMatricula) as 'Total de matrículas', nomeCurso as 'Curso' from tbMatricula
		inner join tbTurma on tbTurma.codTurma = tbMatricula.codTurma
	...
	--13--
		select nomeAluno as 'Alunos que cursam alemão' from tbAluno
		inner join tbMatricula on tbMatricula.codAluno = tbAluno.codAluno
		inner join tbTurma on tbTurma.codTurma = tbMatricula.codTurma
		inner join tbCurso on tbTurma.codCurso = tbCurso.codCurso
